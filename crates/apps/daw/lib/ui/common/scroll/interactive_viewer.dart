...
class CustomInteractiveViewer extends StatefulWidget {
  ...
  final bool useMouseWheelPan;
  final void Function(Offset)? onScroll;
...
class _CustomInteractiveViewerState extends State<CustomInteractiveViewer>
  ...
  void _receivedPointerSignal(PointerSignalEvent event) {
    if (event is PointerScrollEvent) {
      ...
      if (widget.useMouseWheelPan) {
        _receivedPointerSignalForPan(event);
        return;
      }
  ...
  void _receivedPointerSignalForPan(PointerScrollEvent event) {
    final Offset translationChange = event.scrollDelta.scale(-1, -1);

    _panAxis ??= _getPanAxis(Offset.zero, translationChange);
    _transformationController!.value = _matrixTranslate(
      _transformationController!.value,
      translationChange,
    );
    ...
    if (widget.onScroll != null) {
      widget.onScroll!(translationChange);
    }
  }
