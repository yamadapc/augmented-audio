...
private extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_augmented_b494_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_augmented_b494_rustbuffer_free(self, $0) }
    }
}
...
public func initializeLogger() {
    try!
        ...
        rustCall {
            augmented_b494_initialize_logger($0)
        }
}
...
public class AudioOptionsService: AudioOptionsServiceProtocol {
    ...
    public convenience init() {
        self.init(unsafeFromRawPointer: try!
            ...
            rustCall {
                augmented_b494_AudioOptionsService_new($0)
            })
    }

    deinit {
        try! rustCall { ffi_augmented_b494_AudioOptionsService_object_free(pointer, $0) }
    }

    public func getAvailableOptions() -> AvailableAudioOptions {
        let _retval = try!
            rustCall {
                augmented_b494_AudioOptionsService_get_available_options(self.pointer, $0)
            }
    ...
    public func setOptions(model: AudioOptions) {
        try!
            rustCall {
                augmented_b494_AudioOptionsService_set_options(self.pointer, model.lower(), $0)
            }
    }
}
...
public class AudioEngineService: AudioEngineServiceProtocol {
    ...
    public convenience init() {
        self.init(unsafeFromRawPointer: try!
            ...
            rustCall {
                augmented_b494_AudioEngineService_new($0)
            })
    }

    deinit {
        try! rustCall { ffi_augmented_b494_AudioEngineService_object_free(pointer, $0) }
    }

    public func start() {
        try!
            rustCall {
                augmented_b494_AudioEngineService_start(self.pointer, $0)
            }
    }
}
...
private extension AvailableAudioOptions {
    static func read(from buf: Reader) throws -> AvailableAudioOptions {
        return try AvailableAudioOptions(
            ...
            inputIds: FfiConverterSequenceString.read(from: buf),
            outputIds: FfiConverterSequenceString.read(from: buf)
        )
    }

    func write(into buf: Writer) {
        FfiConverterSequenceString.write(hostIds, into: buf)
        FfiConverterSequenceString.write(inputIds, into: buf)
        FfiConverterSequenceString.write(outputIds, into: buf)
    }
}
...
private extension AudioOptions {
    static func read(from buf: Reader) throws -> AudioOptions {
        return try AudioOptions(
            ...
            inputId: FfiConverterOptionString.read(from: buf)
        )
    }

    func write(into buf: Writer) {
        FfiConverterOptionString.write(hostId, into: buf)
        FfiConverterOptionString.write(inputId, into: buf)
    }
}
...
// Helper code for AudioEngineService class is found in ObjectTemplate.swift
// Helper code for AudioOptionsService class is found in ObjectTemplate.swift
// Helper code for AudioOptions record is found in RecordTemplate.swift
// Helper code for AvailableAudioOptions record is found in RecordTemplate.swift
 ...
 */
public enum AugmentedLifecycle {
    /**
